...
function closeChatPicker(options={}){state.pickerOpen=false;state.pickerSwitchToken=(state.pickerSwitchToken||0)+1;document.getElementById("chatPicker")?.classList.add("hidden");document.querySelector(".chat-panel")?.classList.remove("picker-open");setPickerToggleIcon(false);if(options.focusInput){const input=document.getElementById("messageInput");requestAnimationFrame(()=>{if(!input)return;input.focus({preventScroll:true});const fallback=input.value.length;const point=Math.min(input.value.length,state.composerSelectionStart??fallback);input.setSelectionRange(point,point);});}}
...
async function startVoiceRecording(){
  ...
  try{
    ...
    voiceChunks=[];voiceRecorder=new MediaRecorder(voiceStream,preferred?{mimeType:preferred}:undefined);
    voiceRecorder.addEventListener("dataavailable",event=>{if(event.data?.size)voiceChunks.push(event.data);});
    voiceRecorder.addEventListener("stop",()=>{const mime=voiceRecorder?.mimeType||"audio/webm",extension=mime.includes("mp4")?"m4a":"webm",blob=new Blob(voiceChunks,{type:mime});voiceStream?.getTracks().forEach(track=>track.stop());voiceStream=null;voiceRecorder=null;voiceChunks=[];button?.classList.remove("recording");if(blob.size){handleAttachmentSelected(new File([blob],"voice-message-"+Date.now()+"."+extension,{type:mime,lastModified:Date.now()}));toast("Voice message ready to send");}});
    voiceRecorder.start(250);button?.classList.add("recording");toast("Recording… tap the microphone again to stop");
  }catch(error){voiceStream?.getTracks().forEach(track=>track.stop());voiceStream=null;voiceRecorder=null;button?.classList.remove("recording");toast(error?.name==="NotAllowedError"?"Microphone permission is required":"Unable to start voice recording");}
}
