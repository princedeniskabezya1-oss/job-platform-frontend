...
function updateCallButtons(){const m=document.getElementById("muteBtn"),c=document.getElementById("cameraBtn"),s=document.getElementById("speakerBtn");m?.classList.toggle("active",state.isMuted);c?.classList.toggle("active",!state.cameraEnabled);s?.classList.toggle("speaker-on",state.speakerEnabled);s?.setAttribute("aria-pressed",String(state.speakerEnabled));s?.setAttribute("aria-label",state.speakerEnabled?"Use normal call audio":"Use loudspeaker");}
