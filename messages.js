...
function messageContentHtml(message){
  if(message.messageType==="meeting"&&message.meetingInvite)return meetingInviteHtml(message);if(message.deletedForEveryone)return '<div class="message-deleted">This message was deleted</div>';const parts=[];if(message.text)parts.push(`<div class="message-text">${esc(message.text)}</div>`);const a=getPrimaryAttachment(message);if(a)parts.push(attachmentHtml(a,message));if(!parts.length)parts.push('<div class="message-text">Message</div>');return parts.join("");
}
