...
function callVideoConstraints(facingMode="user"){return{facingMode:{ideal:facingMode},width:{ideal:1920,min:640},height:{ideal:1080,min:480},frameRate:{ideal:30,max:30},aspectRatio:{ideal:9/16}};}
async function getLocalMedia(type){state.localStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:type==="video"?callVideoConstraints("user"):false});const v=document.getElementById("localVideo");if(v){v.srcObject=state.localStream;v.play?.().catch(()=>{});}state.isMuted=false;state.cameraEnabled=type==="video";state.speakerEnabled=true;updateCallButtons();return state.localStream;}
...
async function startOutgoingCall(type){const target=getCallTarget();if(!target)return;try{closeCallStartMenu();openCallModal();state.currentCall={callId:"call-"+Date.now(),type,direction:"outgoing",targetUserId:target.userId,targetName:target.name,targetAvatar:target.avatar,conversationId:target.conversationId,status:"ringing"};state.callParticipants.set(String(target.userId),target);updateCallUI({name:target.name,avatar:target.avatar,status:type==="video"?"Starting camera...":"Ringing...",type,waiting:true});safePlay(state.outgoingTone,true);await getLocalMedia(type);updateCallUI({name:target.name,avatar:target.avatar,status:"Ringing...",type,waiting:true});createCallLog(type);await inviteCallUser(target);}catch(e){toast(e.message||"Unable to start call");cleanupCall();}}
...
function updateCallButtons(){const m=document.getElementById("muteBtn"),c=document.getElementById("cameraBtn"),s=document.getElementById("speakerBtn"),modal=document.getElementById("callModal");m?.classList.toggle("active",state.isMuted);c?.classList.toggle("active",!state.cameraEnabled);s?.classList.toggle("speaker-on",state.speakerEnabled);s?.setAttribute("aria-pressed",String(state.speakerEnabled));s?.setAttribute("aria-label",state.speakerEnabled?"Use normal call audio":"Use loudspeaker");modal?.classList.toggle("camera-off",!state.cameraEnabled);}
...
async function toggleCamera(){try{let track=state.localStream?.getVideoTracks()[0];if(!track){const stream=await navigator.mediaDevices.getUserMedia({video:callVideoConstraints("user"),audio:false});track=stream.getVideoTracks()[0];state.localStream.addTrack(track);for(const pc of state.peerConnections.values()){const sender=pc.getSenders().find(s=>s.track?.kind==="video"||s.track===null&&pc.getTransceivers().find(t=>t.sender===s&&t.receiver.track.kind==="video"));if(sender)await sender.replaceTrack(track);}const local=document.getElementById("localVideo");if(local){local.srcObject=state.localStream;local.play?.().catch(()=>{});}state.currentCall.type="video";updateCallUI({name:state.currentCall.targetName,avatar:state.currentCall.targetAvatar,status:"Connected",type:"video",waiting:false});}else{track.enabled=!track.enabled;}state.cameraEnabled=track.enabled;updateCallButtons();}catch(e){toast(e?.name==="NotAllowedError"?"Camera permission is required":"Unable to turn on video");}}
async function switchCallCamera(){const current=state.localStream?.getVideoTracks()[0];if(!current)return toggleCamera();const facing=current.getSettings?.().facingMode==="environment"?"user":"environment",button=document.getElementById("switchCameraBtn");try{button?.classList.add("switching");const stream=await navigator.mediaDevices.getUserMedia({video:callVideoConstraints(facing),audio:false}),next=stream.getVideoTracks()[0];await replaceOutgoingVideo(next);state.localStream.removeTrack(current);state.localStream.addTrack(next);current.stop();const local=document.getElementById("localVideo");if(local){local.srcObject=state.localStream;local.play?.().catch(()=>{});}}catch(e){toast(e?.message||"Unable to switch camera");}finally{button?.classList.remove("switching");}}
...
async function toggleSpeaker(){state.speakerEnabled=!state.speakerEnabled;const outputs=Array.from(document.querySelectorAll(".remote-video-tile video"));try{if(typeof HTMLMediaElement.prototype.setSinkId==="function"&&navigator.mediaDevices?.enumerateDevices){const devices=await navigator.mediaDevices.enumerateDevices(),audio=devices.filter(d=>d.kind==="audiooutput"),pattern=state.speakerEnabled?/speaker|loudspeaker/i:/earpiece|receiver|communications/i,target=audio.find(d=>pattern.test(d.label))||audio.find(d=>d.deviceId==="default")||audio[0];if(target)await Promise.all(outputs.map(media=>media.setSinkId(target.deviceId)));}outputs.forEach(media=>{media.muted=false;media.volume=1;});}catch{outputs.forEach(media=>{media.muted=false;media.volume=1;});}updateCallButtons();}
