...
function toggleMoreMessageReactions(event){event?.stopPropagation();document.getElementById("messageReactionBar")?.classList.toggle("expanded");requestAnimationFrame(positionMessageReactionBar);}
async function reactToSelectedMessage(emoji){const entries=selectedMessageEntries(),entry=entries[0];if(entries.length!==1||!entry)return;try{const updated=await apiJSON("/api/messages/react","POST",{messageId:entry.id,emoji});replaceMessage(updated,true);clearSelectedMessage();}catch(error){toast(error.message||"Unable to add reaction");}}
