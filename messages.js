...
function openMessageReactionDetails(id){const message=state.messages.find(item=>String(messageId(item))===String(id)),list=document.getElementById("reactionDetailsList");if(!message||!list)return;const reactions=Array.isArray(message.reactions)?message.reactions:[];list.innerHTML=reactions.length?reactions.map(item=>{const userId=getId(item.user),mine=String(userId)===String(state.myId),name=mine?"You":item.user?.name||"Participant",emoji=String(item.reaction||item.emoji||"");return `<div class="reaction-detail-row"><span class="reaction-detail-emoji">${esc(emoji)}</span><div class="reaction-detail-copy"><strong>${esc(name)}</strong><small>${mine?"Tap remove to clear your reaction":"Reacted to this message"}</small></div>${mine?`<button class="reaction-remove-btn" onclick="removeMessageReaction('${esc(id)}')">Remove</button>`:""}</div>`;}).join(""):'<div class="empty-list">No reactions</div>';document.getElementById("reactionDetailsModal")?.classList.remove("hidden");}
...
async function sendMessage(){
  ...
  const form=new FormData();form.append("receiverId",rid);if(text)form.append("text",text);if(files.length===1)form.append("file",files[0]);else files.forEach(item=>form.append("files",item,item.name));if(reply?.id)form.append("replyTo",reply.id);form.append("clientMessageId",tempId);if(input)input.value="";autoGrowComposer();clearAttachment();clearReply();
