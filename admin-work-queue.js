(() => {
  ...
  function ensureStyle(){
    ...
    style.textContent = `
      ...
      .awq-refresh{min-height:38px;padding:0 14px;border:1px solid #cfd8e3;border-radius:9px;background:#fff;color:#344054;font-weight:800;cursor:pointer}
      .awq-metrics{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}
      .awq-metric{padding:14px;border:1px solid #e3e8ef;border-radius:13px;background:#fff;cursor:pointer;text-align:left}
      .awq-metric span{display:block;color:#667085;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em}.awq-metric strong{display:block;margin-top:5px;color:#101828;font-size:23px}.awq-metric small{display:block;margin-top:4px;color:#98a2b3;font-size:9px}
      .awq-metric.attention{border-color:#fecaca;background:#fffafa}.awq-metric.attention strong{color:#b42318}
      .awq-toolbar{display:grid;grid-template-columns:minmax(180px,1fr) 190px auto;gap:9px;padding:12px;border:1px solid #e3e8ef;border-radius:13px;background:#fff}
  ...
  function ensureSection(){
    ...
    if(!document.getElementById("adminWorkQueueSection")){
      ...
      section.innerHTML = `
        <div class="awq-shell">
          <div class="awq-hero">
            <div><div class="awq-live">Live AIFT operations</div><h2>Admin Work Queue</h2><p>Every item here represents someone waiting on AIFT, a document that needs review, an upcoming meeting, an evaluation, or a negotiation follow-up. Start a ticket to move it into your working list instead of losing it among new submissions.</p></div>
      ...
      section.querySelector("#awqRefresh")?.addEventListener("click",()=>load(true));
      section.querySelector("#awqSearch")?.addEventListener("input",event=>{state.search=event.target.value;render();});
  ...
  function priorityRank(value){ return ({urgent:0,high:1,normal:2,low:3})[value] ?? 2; }
  ...
  function filtered(){
    ...
    return state.tickets
      ...
      .filter(ticket=>!q || [ticket.title,ticket.description,ticket.nextAction,ticket.metadata?.roomTitle,ticket.metadata?.partnershipName,ticket.metadata?.requestTitle].filter(Boolean).join(" ").toLowerCase().includes(q))
      .sort((a,b)=>{
  ...
  function renderMetrics(){
    ...
    host.innerHTML = `
      ...
      <button class="awq-metric" type="button" data-awq-jump="waiting"><span>Waiting</span><strong>${state.summary.waiting || 0}</strong><small>User, counterparty or meeting time</small></button>
  ...
  async function load(force=false){
    ...
    if(!force && state.lastLoadedAt && Date.now()-state.lastLoadedAt < 12000) return;
    state.loading = true;
    const board = document.getElementById("awqBoard");
    try{
      ...
      if(board) board.innerHTML = `<div class="awq-empty">${esc(error.message)}</div>`;
    }finally{
      state.loading = false;
    }
  }
  ...
  async function patchTicket(ticketId,body){
    const data = await api(`/api/admin/work-tickets/${encodeURIComponent(ticketId)}`,{
      method:"PATCH",
  ...
  function openReviewTicket(ticket){
    ...
    const timer = setInterval(()=>{
      tries += 1;
      const row = document.querySelector(`[data-review-case-id="${CSS.escape(String(reviewId))}"]`);
      if(row){ clearInterval(timer); row.click(); }
      else if(tries > 20){ clearInterval(timer); toast("The review case is in Review Center. Use its case number to locate it."); }
    },250);
  }
  ...
  async function openTicket(ticket){
    try{
      ...
      if(current.sourceType === "review_case"){
        openReviewTicket(current);
        return;
      }
      showTicketDetails(current);
  ...
  function startPolling(){
    ...
    state.timer = setInterval(()=>{
      if(document.hidden || !document.getElementById("adminWorkQueueSection") || document.getElementById("adminWorkQueueSection").classList.contains("hidden")) return;
      load(true);
    },20000);
  }
  ...
  function init(){
    ...
    window.addEventListener("focus",()=>{
      if(!document.getElementById("adminWorkQueueSection")?.classList.contains("hidden")) load(true);
    },{passive:true});
    document.addEventListener("visibilitychange",()=>{
      if(!document.hidden && !document.getElementById("adminWorkQueueSection")?.classList.contains("hidden")) load(true);
    });
    window.addEventListener("aift:admin-work-updated",()=>load(true));
