(() => {
  ...
  function signature(){return JSON.stringify({cases:state.cases.map(x=>[x._id,x.status,x.updatedAt,x.decisionNotes]),interests:state.interests.map(x=>[x._id,x.status,x.updatedAt]),rooms:state.rooms.map(x=>[x._id,x.status,x.workflowStage,x.updatedAt])});}
  function announceUpdate(changed){window.dispatchEvent(new CustomEvent("aift:activity-updated",{detail:{changed,cases:state.cases.length,interests:state.interests.length,rooms:state.rooms.length}}));}
