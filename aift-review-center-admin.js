(() => {
  ...
  const state = { cases:[], restrictions:[], status:"", type:"", search:"" };
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .aift-restrictions{margin-bottom:16px;background:#fff;border:1px solid #fecaca;border-radius:14px;overflow:hidden}.aift-restrictions-head{padding:14px 16px;background:#fff7f7;border-bottom:1px solid #fee2e2}.aift-restrictions-head h3{margin:0;color:#991b1b}.aift-restrictions-head p{margin:5px 0 0;color:#7f1d1d;font-size:12px}.aift-restriction-row{display:grid;grid-template-columns:minmax(180px,1fr) minmax(140px,.7fr) minmax(160px,.8fr) auto;gap:12px;align-items:center;padding:13px 16px;border-bottom:1px solid #eef2f7}.aift-restriction-row:last-child{border-bottom:0}.aift-restriction-person strong,.aift-restriction-person span{display:block}.aift-restriction-person span,.aift-restriction-meta{color:#64748b;font-size:11px}.aift-unlock{border:0;border-radius:9px;background:#057642;color:#fff;padding:9px 12px;font-weight:800;cursor:pointer}
      @media(max-width:700px){.aift-restriction-row{grid-template-columns:1fr}.aift-unlock{width:100%}}
  ...
  function ensureUI(){
    ...
    if(nav && !document.getElementById("aiftReviewCenterNav")){
      ...
      button.innerHTML='<svg viewBox="0 0 24 24"><path d="M4 4h16v16H4z"></path><path d="M8 9h8M8 13h8M8 17h5"></path></svg><span>Reviews & Chat Access</span><span id="aiftReviewPendingBadge" style="margin-left:auto;min-width:20px;text-align:center;border-radius:999px;background:#fee2e2;color:#b91c1c;font-size:10px;padding:2px 6px">0</span>';
  ...
  function openReviewCenter(){
    ...
    const title=document.getElementById("adminPageTitle"),subtitle=document.getElementById("adminPageSubtitle"),badge=document.getElementById("adminSectionBadge");if(title)title.textContent="AIFT Review & Chat Access";if(subtitle)subtitle.textContent="Review protected requests and restore messaging access for approved chat-safety cases.";if(badge)badge.textContent="Trust & Approvals";load();
  }
  ...
  function renderRestrictions(){
    const container=document.getElementById("aiftRestrictedAccounts");if(!container)return;
    if(!state.restrictions.length){container.innerHTML='<div class="aift-review-sync">No active messaging restrictions. Temporary restrictions that have expired are released automatically.</div>';return;}
    container.innerHTML=`<section class="aift-restrictions"><div class="aift-restrictions-head"><h3>Restricted messaging accounts (${state.restrictions.length})</h3><p>These members are currently unable to send messages because contact information was repeatedly shared. Review the account and unlock it when appropriate.</p></div>${state.restrictions.map(item=>{const user=item.userId||{};return `<div class="aift-restriction-row"><div class="aift-restriction-person"><strong>${esc(displayName(user))}</strong><span>${esc(user.email||"No email")} · ${esc(titleCase(user.role||"member"))}</span></div><div><span class="aift-review-pill priority-high">${esc(titleCase(item.action))}</span><div class="aift-restriction-meta">Warning ${esc(item.warningNumber||"—")}</div></div><div class="aift-restriction-meta">${item.action==="admin_review"?"Pending admin review":`Restricted until ${esc(fmt(item.restrictedUntil))}`}${item.reviewCaseId?.caseNumber?`<br>${esc(item.reviewCaseId.caseNumber)}`:""}</div><button class="aift-unlock" type="button" data-unlock-restriction="${esc(item._id)}">Unlock messaging</button></div>`}).join("")}</section>`;
    container.querySelectorAll("[data-unlock-restriction]").forEach(button=>button.addEventListener("click",()=>unlockRestriction(button.dataset.unlockRestriction)));
  }

  async function unlockRestriction(id){
    const item=state.restrictions.find(entry=>String(entry._id)===String(id));if(!item)return;
    const run=async()=>{try{await api(`/api/messages/admin/contact-restrictions/${encodeURIComponent(id)}/unlock`,{method:"PATCH",body:JSON.stringify({note:"Messaging access restored from AIFT Admin Review & Chat Access."})});if(typeof window.adminToast==="function")window.adminToast("Messaging access restored.");await load();}catch(error){if(typeof window.adminToast==="function")window.adminToast(error.message);else alert(error.message);}};
    if(typeof window.openAdminConfirm==="function"){window.openAdminConfirm("Unlock messaging",`Restore messaging access for ${displayName(item.userId)}?`,run);return;}await run();
  }
  ...
  function render(){
    renderMetrics();renderRestrictions();const container=document.getElementById("aiftReviewTable");if(!container)return;const rows=filtered();if(!rows.length){container.innerHTML='<div class="aift-review-empty">No AIFT review cases matched these filters.</div>';return;}
  ...
  async function load(){ensureUI();const container=document.getElementById("aiftReviewTable");if(container)container.innerHTML='<div class="aift-review-empty">Loading AIFT review cases…</div>';try{const params=new URLSearchParams();if(state.status)params.set("status",state.status);if(state.type)params.set("type",state.type);const [data,restrictionData]=await Promise.all([api(`/api/review-cases/admin${params.toString()?`?${params}`:""}`),api("/api/messages/admin/contact-restrictions")]);state.cases=Array.isArray(data?.cases)?data.cases:[];state.restrictions=Array.isArray(restrictionData?.restrictions)?restrictionData.restrictions:[];render();}catch(error){if(container)container.innerHTML=`<div class="aift-review-empty">${esc(error.message)}</div>`;}}
  ...
  function workflowCopy(item){
    ...
    if(item.type==="chat_safety") return { approved:"Unlock messaging", matched:"Close safety review", negotiation:"Continue review", completed:"Complete safety review" };
  ...
  async function updateCase(id,status){
    ...
    if(sensitive.includes(status)&&typeof window.openAdminConfirm==="function"){window.openAdminConfirm(`${titleCase(status)} ${item.caseNumber}`,status==="approved"?(item.type==="chat_safety"?"Unlock this member’s messaging access now? The restriction linked to this safety case will be released.":item.type==="investment_interest"?"Approve this investor interest for the next controlled AIFT stage? The venture owner may receive the approved interest, but personal contact information is not released.":"Approve this request for its next controlled AIFT stage? Personal contact information is not released."):`This will mark the AIFT review as ${titleCase(status)}. Continue?`,run);return;}await run();
  }
