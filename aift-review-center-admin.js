(() => {
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .aift-review-table-wrap{background:#fff;border:1px solid #e3e8ef;border-radius:14px;overflow:auto}.aift-review-table{width:100%;border-collapse:collapse;min-width:980px}.aift-review-table th,.aift-review-table td{text-align:left;padding:12px 14px;border-bottom:1px solid #edf1f5;font-size:12px;vertical-align:middle}.aift-review-table th{background:#f8fafc;color:#475569;font-size:11px;text-transform:uppercase;letter-spacing:.04em}.aift-review-row{cursor:pointer}.aift-review-row:hover{background:#f8fbff}.aift-review-case{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:#0a66c2;font-weight:800}.aift-review-title{font-weight:800;color:#0f172a}.aift-review-muted{color:#64748b}.aift-review-pill{display:inline-flex;padding:4px 8px;border-radius:999px;background:#eef2ff;color:#4338ca;font-size:10px;font-weight:800;text-transform:capitalize}.aift-review-pill.priority-urgent,.aift-review-pill.priority-high{background:#fee2e2;color:#b91c1c}.aift-review-empty{padding:34px;text-align:center;color:#64748b}.aift-review-actions{display:flex;gap:8px;flex-wrap:wrap}.aift-review-actions button{border:1px solid #cbd5e1;background:#fff;border-radius:8px;padding:8px 10px;font-weight:700;cursor:pointer}.aift-review-actions button.primary{background:#0a66c2;color:#fff;border-color:#0a66c2}.aift-review-actions button.danger{background:#fff1f2;color:#be123c;border-color:#fecdd3}.aift-review-detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:16px}.aift-review-detail{border:1px solid #e5e7eb;border-radius:10px;padding:12px}.aift-review-detail span{display:block;color:#64748b;font-size:11px;margin-bottom:4px}.aift-review-history{margin-top:16px}.aift-review-history-item{padding:10px 0;border-bottom:1px solid #eef2f7}.aift-review-history-item strong{display:block;font-size:12px}.aift-review-history-item span{font-size:11px;color:#64748b}.aift-review-note{width:100%;min-height:90px;border:1px solid #cbd5e1;border-radius:10px;padding:10px;font:inherit;margin-top:12px}
  ...
  function ensureUI(){
    ...
    if(nav && !document.getElementById("aiftReviewCenterNav")){
      ...
      const verification=nav.querySelector('[data-section="verification"]');
      nav.insertBefore(button,verification||nav.children[1]||null);
      button.addEventListener("click",openReviewCenter);
    }
    ...
    if(main && !document.getElementById("aiftReviewCenterSection")){
      ...
      section.id="aiftReviewCenterSection";
      ...
      const contentHost=document.querySelector("#overviewSection")?.parentElement || main;
      contentHost.appendChild(section);
      section.querySelector("#aiftReviewSearch")?.addEventListener("input",e=>{state.search=e.target.value;render();});
      section.querySelector("#aiftReviewStatus")?.addEventListener("change",e=>{state.status=e.target.value;load();});
      section.querySelector("#aiftReviewType")?.addEventListener("change",e=>{state.type=e.target.value;load();});
      section.querySelector("#aiftReviewRefresh")?.addEventListener("click",load);
      section.addEventListener("click",e=>{const row=e.target.closest("[data-review-case-id]");if(row) openCase(row.dataset.reviewCaseId);});
    }
  }
  ...
  function openReviewCenter(){
    ...
    document.getElementById("aiftReviewCenterSection")?.classList.remove("hidden");
    document.querySelectorAll(".admin-nav button").forEach(btn=>btn.classList.toggle("active",btn.id==="aiftReviewCenterNav"));
    const title=document.getElementById("adminPageTitle"),subtitle=document.getElementById("adminPageSubtitle"),badge=document.getElementById("adminSectionBadge");
    if(title) title.textContent="AIFT Review Center";
    if(subtitle) subtitle.textContent="Review Career Hub, funding, investment, partnership, scholarship and safety cases before parties can proceed.";
    if(badge) badge.textContent="Trust & Approvals";
    load();
  }

  function renderMetrics(){
    const counts={submitted:0,under_review:0,information_requested:0,urgent:0,open:0};
    state.cases.forEach(item=>{if(counts[item.status]!==undefined)counts[item.status]+=1;if(["urgent","high"].includes(item.priority))counts.urgent+=1;if(!["approved","rejected","completed","cancelled","expired"].includes(item.status))counts.open+=1;});
    const metrics=document.getElementById("aiftReviewMetrics");
    if(metrics) metrics.innerHTML=`<div class="aift-review-metric"><span>Open</span><strong>${counts.open}</strong></div><div class="aift-review-metric"><span>Submitted</span><strong>${counts.submitted}</strong></div><div class="aift-review-metric"><span>Under Review</span><strong>${counts.under_review}</strong></div><div class="aift-review-metric"><span>Needs Info</span><strong>${counts.information_requested}</strong></div><div class="aift-review-metric"><span>High Priority</span><strong>${counts.urgent}</strong></div>`;
    const badge=document.getElementById("aiftReviewPendingBadge");if(badge)badge.textContent=String(counts.open);
  }

  function filtered(){
    const q=state.search.trim().toLowerCase();
    if(!q)return state.cases;
    return state.cases.filter(item=>[item.caseNumber,item.title,item.type,item.status,displayName(item.requesterId)].filter(Boolean).join(" ").toLowerCase().includes(q));
  }

  function render(){
    renderMetrics();
    const container=document.getElementById("aiftReviewTable");
    if(!container)return;
    const rows=filtered();
    if(!rows.length){container.innerHTML='<div class="aift-review-empty">No AIFT review cases matched these filters.</div>';return;}
    container.innerHTML=`<table class="aift-review-table"><thead><tr><th>Case</th><th>Type</th><th>Request</th><th>Requester</th><th>Status</th><th>Priority</th><th>Submitted</th></tr></thead><tbody>${rows.map(item=>`<tr class="aift-review-row" data-review-case-id="${esc(item._id)}"><td class="aift-review-case">${esc(item.caseNumber)}</td><td>${esc(titleCase(item.type))}</td><td><div class="aift-review-title">${esc(item.title)}</div><div class="aift-review-muted">${esc(String(item.summary||"").slice(0,90))}</div></td><td>${esc(displayName(item.requesterId))}<div class="aift-review-muted">${esc(item.requesterId?.role||"")}</div></td><td><span class="aift-review-pill">${esc(titleCase(item.status))}</span></td><td><span class="aift-review-pill priority-${esc(item.priority||"normal")}">${esc(titleCase(item.priority||"normal"))}</span></td><td>${esc(fmt(item.createdAt))}</td></tr>`).join("")}</tbody></table>`;
  }

  async function load(){
    ensureUI();
    const container=document.getElementById("aiftReviewTable");if(container)container.innerHTML='<div class="aift-review-empty">Loading AIFT review cases…</div>';
    try{
      const params=new URLSearchParams();if(state.status)params.set("status",state.status);if(state.type)params.set("type",state.type);
      const data=await api(`/api/review-cases/admin${params.toString()?`?${params}`:""}`);
      state.cases=Array.isArray(data?.cases)?data.cases:[];
      render();
    }catch(error){if(container)container.innerHTML=`<div class="aift-review-empty">${esc(error.message)}</div>`;}
  }

  function currentCase(id){return state.cases.find(item=>String(item._id)===String(id));}

  function openCase(id){
    const item=currentCase(id);if(!item)return;
    const history=Array.isArray(item.history)?item.history:[];
    const html=`<div class="aift-review-detail-grid"><div class="aift-review-detail"><span>Case number</span><strong>${esc(item.caseNumber)}</strong></div><div class="aift-review-detail"><span>Type</span><strong>${esc(titleCase(item.type))}</strong></div><div class="aift-review-detail"><span>Requester</span><strong>${esc(displayName(item.requesterId))}</strong></div><div class="aift-review-detail"><span>Status</span><strong>${esc(titleCase(item.status))}</strong></div><div class="aift-review-detail"><span>Resource</span><strong>${esc(item.resourceType||"—")}</strong></div><div class="aift-review-detail"><span>Priority</span><strong>${esc(titleCase(item.priority||"normal"))}</strong></div></div><h4 style="margin:0 0 6px">${esc(item.title)}</h4><p style="color:#475569;line-height:1.6">${esc(item.summary||"No summary provided.")}</p><textarea class="aift-review-note" id="aiftReviewDecisionNote" placeholder="Internal review note / information request / decision reason"></textarea><div class="aift-review-history"><strong>Case history</strong>${history.length?history.slice().reverse().map(h=>`<div class="aift-review-history-item"><strong>${esc(titleCase(h.status))}</strong><span>${esc(h.note||"")} · ${esc(fmt(h.createdAt))}</span></div>`).join(""):'<div class="aift-review-empty">No history yet.</div>'}</div>`;
    const actions=`<div class="aift-review-actions"><button type="button" data-review-action="under_review" data-id="${esc(id)}">Start review</button><button type="button" data-review-action="information_requested" data-id="${esc(id)}">Request information</button><button type="button" class="primary" data-review-action="approved" data-id="${esc(id)}">Approve</button><button type="button" data-review-action="matched" data-id="${esc(id)}">Mark matched</button><button type="button" data-review-action="negotiation" data-id="${esc(id)}">Negotiation</button><button type="button" data-review-action="completed" data-id="${esc(id)}">Complete</button><button type="button" class="danger" data-review-action="rejected" data-id="${esc(id)}">Reject</button></div>`;
    if(typeof window.openAdminReviewModal==="function") window.openAdminReviewModal(item.caseNumber,item.title,html,actions);
    else alert(`${item.caseNumber}\n${item.title}\n${item.status}`);
  ...
  async function updateCase(id,status){
    const note=String(document.getElementById("aiftReviewDecisionNote")?.value||"").trim();
    try{
      await api(`/api/review-cases/${encodeURIComponent(id)}/admin`,{method:"PATCH",body:JSON.stringify({status,note})});
      if(typeof window.closeAdminReviewModal==="function")window.closeAdminReviewModal();
      if(typeof window.adminToast==="function")window.adminToast(`Review case updated: ${titleCase(status)}`);
      await load();
    }catch(error){if(typeof window.adminToast==="function")window.adminToast(error.message);else alert(error.message);}
  }

  async function refreshBadge(){
    try{const data=await api("/api/review-cases/admin");state.cases=Array.isArray(data?.cases)?data.cases:[];renderMetrics();}catch{}
  }

  function init(){
    if(String(localStorage.getItem("role")||"").toLowerCase()!=="admin"||!token())return;
    ensureStyle();ensureUI();setTimeout(refreshBadge,900);
  }
