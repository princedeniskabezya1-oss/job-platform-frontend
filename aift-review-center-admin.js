(() => {
  ...
  function actionButtons(item){
    ...
    const id=esc(item._id);return `<div class="aift-review-actions"><button type="button" data-review-action="under_review" data-id="${id}">Start review</button><button type="button" data-review-action="information_requested" data-id="${id}">Request information</button><button type="button" class="primary" data-review-action="approved" data-id="${id}">Approve & release</button><button type="button" data-review-action="matched" data-id="${id}">Mark matched</button><button type="button" data-review-action="negotiation" data-id="${id}">Negotiation</button><button type="button" data-review-action="completed" data-id="${id}">Complete</button><button type="button" class="danger" data-review-action="rejected" data-id="${id}">Reject</button></div>`;
  }
  ...
  async function updateCase(id,status){
    ...
    if(sensitive.includes(status)&&typeof window.openAdminConfirm==="function"){window.openAdminConfirm(`${titleCase(status)} ${item.caseNumber}`,status==="approved"?"This can release the linked resource to the receiving party. Continue?":`This will mark the AIFT review as ${titleCase(status)}. Continue?`,run);return;}await run();
  }
